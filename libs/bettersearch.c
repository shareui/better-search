...
EXPORT float msg_score(const char *text, const char *query) {
    if (!text || !text[0] || !query || !query[0]) return 0.0f;

    char tl[MAX_FIELD_LEN], ql[MAX_FIELD_LEN];
    utf8_lower(text,  tl, sizeof(tl));
    utf8_lower(query, ql, sizeof(ql));
    ...
    if (translit(ql, tq, sizeof(tq)) && strcmp(tq, ql) != 0) {
        ...
        for (int i = 0; tq[i]; i++) if (tq[i] != ' ') no_space[k++] = tq[i];
        no_space[k] = '\0';
        if (is_alpha_or_space(no_space)) {
            if (strstr(tl, tq)) return 0.95f;
        }
    }
    ...
    if (tokenN > 1) {
        ...
        for (int i = 0; i < tokenN; i++) {
            if (!strstr(tl, tokens[i].w)) { all = 0; break; }
        }
        if (all) return 0.85f;
    }

    // trigram
    uint32_t qtri[MAX_TRIGRAMS], ttri[MAX_TRIGRAMS];
    int qtrin, ttrin;
    {
        uint32_t tmp[MAX_TRIGRAMS];
        int n = make_trigrams(ql, tmp, MAX_TRIGRAMS);
    ...
    {
        uint32_t tmp[MAX_TRIGRAMS];
        int n = make_trigrams(tl, tmp, MAX_TRIGRAMS);
        ttrin = sort_dedup(tmp, n);
        memcpy(ttri, tmp, ttrin * sizeof(uint32_t));
    }
    float sim = trigram_similarity(qtri, qtrin, ttri, ttrin);
    return sim >= MIN_SIMILARITY ? sim * 0.7f : 0.0f;
}

// dialogs 

EXPORT float dialog_score(const char *name, const char *username,
                           const char *about, const char *query) {
    if (!query || !query[0]) return 0.0f;
    ...
    if (translit(ql, tq, sizeof(tq)) && strcmp(tq, ql) != 0) {
        ...
        for (int i = 0; tq[i]; i++) if (tq[i] != ' ') no_space[k++] = tq[i];
        no_space[k] = '\0';
        if (is_alpha_or_space(no_space)) translitDone = 1;
        else tq[0] = '\0';
    }

    char nl[MAX_FIELD_LEN], ul[MAX_FIELD_LEN], al[MAX_FIELD_LEN];
    utf8_lower(name     ? name     : "", nl, sizeof(nl));
    utf8_lower(username ? username : "", ul, sizeof(ul));
    utf8_lower(about    ? about    : "", al, sizeof(al));

    if (strstr(nl, ql)) return 1.0f;
    if (strstr(ul, ql)) return 0.95f;
    if (strstr(al, ql)) return 0.8f;

    if (translitDone) {
        if (strstr(nl, tq)) return 0.92f;
        if (strstr(ul, tq)) return 0.87f;
        if (strstr(al, tq)) return 0.72f;
    }

    Word nameWords[MAX_WORDS]; int nameWordsN = split_words(nl, nameWords, MAX_WORDS);
    for (int i = 0; i < nameWordsN; i++) {
        if (strncmp(nameWords[i].w, ql, strlen(ql)) == 0) return 0.85f;
    }

    uint32_t qtri[MAX_TRIGRAMS], ntri[MAX_TRIGRAMS];
    int qtrin, ntrin;
    {
        uint32_t tmp[MAX_TRIGRAMS];
        int n = make_trigrams(ql, tmp, MAX_TRIGRAMS);
    ...
    {
        uint32_t tmp[MAX_TRIGRAMS];
        int n = make_trigrams(nl, tmp, MAX_TRIGRAMS);
        ntrin = sort_dedup(tmp, n);
        memcpy(ntri, tmp, ntrin * sizeof(uint32_t));
    }
    float nameSim = trigram_similarity(qtri, qtrin, ntri, ntrin);
    if (nameSim >= MIN_SIMILARITY) return nameSim * 0.75f;
